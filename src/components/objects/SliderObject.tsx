...
const Item = ({ item }: { item: ItemSchemaType }) => {
  const getPublishedDate = () => {
    switch (item._type) {
      ...
      default:
        return (
          new Date(item.originalPublishedAt).getTime() ??
          new Date(item._createdAt).getTime()
        )
    }
  }
  const href = utilityService.getHref({
...
const SliderObject = ({ object }: { object: SliderObjectSchemaType }) => {
  ...
  const categoryItems =
    category?.items?.map((item, index): ItemSchemaType => {
      return {
        _id: item._id,
        _type: item._type,
        featuredImage: item.featuredImage ?? {
