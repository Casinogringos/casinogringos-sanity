...
const BonusObject = ({ object }: { object: BonusObjectSchemaType }) => {
    const { casino, bonus, freeSpins, terms, title, information, buttonText } = object
    let getBonusText = () => {
        let bonusText = ''
    ...
    return <div className="not-prose bg-dark p-4 rounded-md flex flex-col items-center">
        <Image
            src={casino.logo.src}
        ...
        />
        {bonusText && <span className="text-white text-2xl mb-4">{bonusText}</span>}
        <Link href={casino.slug.current} variant="affiliate" className="text-white">{buttonText ?? `Till ${casino.name} casino`}</Link>
    </div>
}
