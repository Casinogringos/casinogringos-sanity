import CasinoListClient from '@/src/components/casino/CasinoListClient'
...
import {
  extractPaymentMethodOptions,
  mapCasinoToDTO,
} from '@/src/lib/mapCasinoToDTO'
...
export default function CasinoList({
  ...
  const casinoDTOs = casinos
    .filter((casino) => !casino.excludeFromToplists)
    .map((casino) => mapCasinoToDTO(casino, categoryList))

  const paymentMethodOptions = extractPaymentMethodOptions(casinos)
