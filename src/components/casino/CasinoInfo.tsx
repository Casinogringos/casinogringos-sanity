...
const CasinoInfo = ({ casinoPage }: { casinoPage: CasinoPageSchemaType }) => {
  ...
  return (
    <div>
      <div className="mb-8 w-full bg-slate-100 border border-slate-200 p-8 pb-6 rounded-2xl">
        <div className="relative mb-8 gap-6 overflow-hidden rounded-t-md bg-darklight p-5 md:overflow-visible md:rounded-md">
          ...
          <div className="mt-2 flex items-start text-white">
            ...
            <Image
              src={
                finalRating >= 4 ? '/gringo-happy.webp' : '/gringo-neutral.webp'
              }
              width={160}
              height={160}
          ...
          {casinoPage.ratingMotivation && (
            <div className="mr-28 pt-3 text-sm text-slate-200 md:mr-36">
              <PortableText value={casinoPage.ratingMotivation} />
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className={'mb-5 flex flex-wrap items-center'}>
            {paymentMethodPages.map(({ paymentMethod, linkedPage }, i) => {
              ...
              return (
                <Tag
                  href={linkedPage?.slug.current}
                  className="h-[40px] w-[54px] relative"
                  key={`payment-provider-${paymentMethod._id}`}
                >
