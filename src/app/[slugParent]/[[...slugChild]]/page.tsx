...
import { getPostPreviews, getNodeByUri, getStaticParams } from '@/lib/api'
import { notFound } from 'next/navigation'
import CasinoPage from '@/src/app/CasinoPage'
import { extractSlugFromUrl } from '@/lib/helpers'
import { Metadata } from 'next'
import { Post as PostType, Page as PageType } from '@/types/index'
...
export default async function Page(props: { params: Params }) {
  ...
  const page = (await getNodeByUri({ uri: pageUri })) as PageType
  if (page?.__typename === 'Page') {
    return <SubPage page={page} />
  }
  const post = (await getNodeByUri({
    uri: slugParent,
  })) as PostType
  if (!post) {
    return notFound()
