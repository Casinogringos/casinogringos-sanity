import { getNodeByUri, getSlotPreviews, getStaticParams } from "@/lib/api";
import { notFound } from "next/navigation";
import Slot from "@/src/app/Slot";
import { Slot as SlotType } from "@/types/index";
...
export default async function SlotItem(props: { params: Params }) {
  const params = await props.params;
  const slot = (await getNodeByUri({
    uri: `/slots/${params?.slug}`,
  })) as SlotType;
  ...
  if (slot) {
    return <Slot slot={slot} similarSlots={similarSlots} />;
