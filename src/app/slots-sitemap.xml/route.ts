...
import { SubPageSchemaType, SlotPageSchemaType } from '@/src/schemas'
...
export async function GET() {
    ...
    const slotsPagesResponse: SlotPageSchemaType[] = await getSitemap('slot-pages')
    ...
    const indexPage = () => {
        const featuredImage =
            slotsIndexResponse.seoImage?.src
        const contentImages = imageService.getImagesFromModularContent(slotsIndexResponse.content)
        const allImages = featuredImage
            ? [featuredImage, ...contentImages]
            : contentImages
        const imagesXML = imageService.getImagesXML(allImages).filter((image) => image !== null)
